...
class AutomacaoSEIApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="— Inclusão de Processos Vr03", fg=TEXT2, bg=BG3,
        ...
        # Cabeçalho da seção com botão LIMPAR na mesma linha
        ...
        # Mini contagem regressiva do intervalo
    ...
    def limpar_lista(self):
        """Limpa a área de texto da lista de processos."""
    ...
    def _run_countdown(self, seconds=3):
        """Contagem regressiva exibida no painel PRÓXIMO ITEM EM."""
        self._stop_flag = False
    ...
    def _run_interval_countdown(self, seconds):
        """Contagem regressiva inline no campo de intervalo."""
        for i in range(seconds, 0, -1):
            if self._stop_flag or not self.is_running:
    ...
    def _play_completion_sound(self):
        """Emite beeps de finalização usando winsound."""
        try:
            for _ in range(3):
            ...
            pass  # Silencia erros caso winsound não esteja disponível

    def _start_blink(self):
        """Inicia o efeito de piscar no label de status."""
        self._blink_state = True
        self._do_blink()

    def _do_blink(self):
        """Alterna entre duas cores para o efeito de piscar."""
    ...
    def _run(self):
        ...
        try:
            while self.index_atual < len(self.dados) and self.is_running:
                ...
                # Contagem regressiva do intervalo (exibida inline)
                ...
                # Segunda contagem regressiva do intervalo
                self._run_interval_countdown(intervalo)
                if self._stop_flag or not self.is_running:
                    break
            ...
            if self.is_running:
                # ✅ Finalização: som + mensagem piscando
