"""
╔══════════════════════════════════════════════════════════════════╗
║      Mesclador de Comprovantes de Pagamento  v8.1                ║
║  - Se COMPROVANTE inexistente, nenhum arquivo é gerado           ║
║  - ISS/IRRF buscados por keyword NO PDF de comprovantes          ║
║    (comportamento original v7 restaurado)                        ║
║  - NOVO (19/03/2026): também mescla arquivo físico ISS/IR        ║
║    localizado na pasta pelo prefixo numérico do nome             ║
║  - Comentários e marcadores FIM em todas as seções               ║
╠══════════════════════════════════════════════════════════════════╣
║  Ordem de mesclagem:                                             ║
║  AUTORIZAÇÃO + COMPROVANTE + ISS(keyword) + IRRF(keyword)        ║
║  + BOLETO_ISS(arquivo) + BOLETO_IR(arquivo) + CERTIDÕES          ║
╠══════════════════════════════════════════════════════════════════╣
...
def paginas_keyword(pdf_path, kw):
    ...
    (Comportamento original v7 — restaurado em 19/03/2026)
    """
    reader = PdfReader(pdf_path)
...
# ═══════════════════════════════════════════════════════════════════
#  Alterei aqui em 19/03/2026
#  BUSCA DE ARQUIVO FÍSICO ISS/IR NA PASTA POR PREFIXO NUMÉRICO
#
#  Complementa (NÃO substitui) a busca por keyword no PDF de
#  comprovantes. Localiza na pasta um arquivo PDF cujo nome comece
#  com o mesmo prefixo numérico da Autorização/Comprovante e
#  contenha "ISS" ou "IR" no nome.
#
#  Exemplos de arquivos localizados:
#    "05-144.005.261-9 ISS - HRG RC- BOLETO OK.pdf"
#    "05-144.005.261-9 IR - HRG RC- BOLETO OK.pdf"
#    "06-144.008.239-9 IR -HRG RI- BOLETO OK.pdf"
#
#  O arquivo encontrado é incluído COMPLETO na mesclagem,
#  posicionado APÓS os comprovantes de imposto (keyword) e
#  ANTES das certidões.
# ═══════════════════════════════════════════════════════════════════

def buscar_arquivo_imposto(pasta, prefixo_numerico, tipo_imposto):
    """
    Procura na 'pasta' um arquivo PDF que:
      1. Comece com o mesmo prefixo numérico (ex: '05-144.005.261-9 ')
      2. Contenha o tipo_imposto no nome ('ISS' ou 'IR')
         — para 'IR' usa \\bIR\\b para não confundir com outras palavras

    Retorna o caminho completo do primeiro arquivo encontrado, ou None.

    Parâmetros:
        pasta            : caminho da pasta onde buscar
        prefixo_numerico : string do prefixo (ex: '05-144.005.261-9 ')
        tipo_imposto     : 'ISS' ou 'IR'
    """
    try:
    ...
    prefixo_norm = sem_acento(prefixo_numerico).rstrip()
    ...
    for nome in sorted(arquivos):
        ...
        # Verifica se começa com o prefixo numérico
        if not nome_norm.startswith(prefixo_norm):
            continue

        # Pula arquivos que são a própria Autorização ou Comprovantes
        if re.search(r"AUTORIZACAO|COMPROVANTES", nome_norm):
            continue

        # Verifica se contém o tipo de imposto no nome
        if tipo_imposto.upper() == "IR":
            # \bIR\b: evita match em "DIRETORIA", "IRRF", etc.
            if re.search(r"\bIR\b", nome_norm):
                return os.path.join(pasta, nome)
        else:
            # ISS: busca simples pela string
            if "ISS" in nome_norm:
                return os.path.join(pasta, nome)

    return None

# FIM Alterei aqui em 19/03/2026 — BUSCA DE ARQUIVO FÍSICO ISS/IR NA PASTA
...
class App:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Mesclador de Comprovantes de Pagamento  v8.1")
    ...
    def _processar(self):
        ...
        5. SE comprovante OK: prossegue com os demais
           SE comprovante INEXISTENTE: bloqueia TUDO para aquele registro

        Ordem de mesclagem (v8.1):
          AUTORIZAÇÃO (completa)
          + COMPROVANTE (página(s) do PDF de comprovantes — original v7)
          + ISS keyword (página(s) com 'ISS' no PDF de comprovantes — original v7)
          + IRRF keyword (página(s) com 'IRRF' no PDF de comprovantes — original v7)
          + BOLETO ISS (arquivo físico da pasta pelo prefixo — NOVO 19/03/2026)
          + BOLETO IR  (arquivo físico da pasta pelo prefixo — NOVO 19/03/2026)
          + CERTIDÕES (arquivos completos)
        """
        ...
        for nome_aut in arqs_aut:
            ...
            # ── Alterei aqui em 19/03/2026 ──────────────────────────────
            # Pré-busca dos arquivos físicos de BOLETO ISS e IR na pasta.
            # Feita uma vez por Autorização pois todos os registros da
            # mesma conta compartilham os mesmos arquivos de boleto.
            # Resultado: None se não existir, caminho completo se existir.
            # NÃO bloqueia a mesclagem — é um acréscimo OPCIONAL.
            # Se o arquivo existir, é incluído completo após os comprovantes
            # de imposto por keyword e antes das certidões.
            arq_boleto_iss = buscar_arquivo_imposto(pasta_comp, prefixo, "ISS")
            arq_boleto_ir  = buscar_arquivo_imposto(pasta_comp, prefixo, "IR")

            if arq_boleto_iss:
                self.lok(f"Boleto ISS localizado: {os.path.basename(arq_boleto_iss)}")
            else:
                self.ldbg(f"Boleto ISS nao encontrado na pasta (prefixo '{prefixo.strip()}')")

            if arq_boleto_ir:
                self.lok(f"Boleto IR localizado: {os.path.basename(arq_boleto_ir)}")
            else:
                self.ldbg(f"Boleto IR nao encontrado na pasta (prefixo '{prefixo.strip()}')")
            # FIM Alterei aqui em 19/03/2026
            ...
            for r in regs:
                ...
                # ── Passo 9 – busca página ISS no PDF de comprovantes ─
                # (comportamento original v7 — restaurado em 19/03/2026)
                pags_iss = []
                if precisa_iss:
                    try:
                    ...
                    if pags_iss:
                        self.lok(f"Passo 9 - ISS (keyword): {len(pags_iss)} pag(s)")
                    else:
                        status["iss"] = "INEXISTENTE"
                        self.lwrn("Passo 9 - ISS INEXISTENTE (keyword nao encontrada)")
                # FIM Passo 9

                # ── Passo 10 – busca página IRRF no PDF de comprovantes
                # (comportamento original v7 — restaurado em 19/03/2026)
                pags_irrf = []
                if precisa_irrf:
                    try:
                    ...
                    if pags_irrf:
                        self.lok(f"Passo 10 - IRRF (keyword): {len(pags_irrf)} pag(s)")
                    else:
                        status["irrf"] = "INEXISTENTE"
                        self.lwrn("Passo 10 - IRRF INEXISTENTE (keyword nao encontrada)")
                # FIM Passo 10
                ...
                # Verifica se ainda há algum INEXISTENTE obrigatório
                ...
                # ── Alterei aqui em 19/03/2026 ──────────────────────────
                # Monta lista de páginas dos boletos físicos ISS/IR.
                # Estes são OPCIONAIS — se o arquivo existir na pasta,
                # é incluído completo; se não existir, simplesmente não
                # entra na mesclagem sem bloquear nada.
                pags_boleto_iss = [(arq_boleto_iss, -1)] if arq_boleto_iss else []
                pags_boleto_ir  = [(arq_boleto_ir,  -1)] if arq_boleto_ir  else []

                if pags_boleto_iss:
                    self.lok(f"Passo 12 - Boleto ISS incluido na mesclagem")
                if pags_boleto_ir:
                    self.lok(f"Passo 12 - Boleto IR incluido na mesclagem")
                # FIM Alterei aqui em 19/03/2026

                # ── Mesclagem na ordem correta ────────────────────────────
                # AUTORIZAÇÃO (completa)
                # + COMPROVANTE (página(s) — original v7)
                # + ISS keyword (página(s) — original v7)
                # + IRRF keyword (página(s) — original v7)
                # + BOLETO ISS (arquivo físico — NOVO 19/03/2026)
                # + BOLETO IR  (arquivo físico — NOVO 19/03/2026)
                # + CERTIDÕES (arquivos completos)
                todas_pags = ([(caminho_aut, -1)]   # Autorização completa
                              + pags_comp            # página(s) do comprovante
                              + pags_iss             # página(s) ISS keyword
                              + pags_irrf            # página(s) IRRF keyword
                              + pags_boleto_iss      # boleto ISS arquivo físico (NOVO)
                              + pags_boleto_ir       # boleto IR arquivo físico (NOVO)
