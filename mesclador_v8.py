"""
╔══════════════════════════════════════════════════════════════════╗
║      Mesclador de Comprovantes de Pagamento  v8.0                ║
║  - Se COMPROVANTE inexistente, nenhum arquivo é gerado           ║
║  - ISS/IR buscados por ARQUIVO NA PASTA (mesmo prefixo numérico) ║
...
#  BUSCA DE ARQUIVO ISS/IR NA PASTA POR PREFIXO NUMÉRICO
#
#  Lógica antiga: buscava a keyword "ISS" ou "IRRF" dentro das páginas
#  do PDF de comprovantes (função paginas_keyword).
#
#  Lógica nova: busca na PASTA um arquivo PDF cujo nome comece com o
#  mesmo prefixo numérico da Autorização/Comprovante e contenha a
#  palavra "ISS" ou "IR" no nome.
#
#  Exemplo:
#    Autorização  → "05-144.005.261-9 Autorizacao SEI.pdf"
#    Comprovantes → "05-144.005.261-9 COMPROVANTES.pdf"
#    ISS          → "05-144.005.261-9 ISS - HRG RC- BOLETO OK.pdf"  ✔
#    IR           → "05-144.005.261-9 IR - HRG RC- BOLETO OK.pdf"   ✔
#
#  O prefixo numérico é extraído do nome da Autorização
#  (tudo que vier antes de "Autorizacao", incluindo os traços/pontos).
# ═══════════════════════════════════════════════════════════════════
...
def buscar_arquivo_imposto(pasta, prefixo_numerico, tipo_imposto):
    ...
    Procura na 'pasta' um arquivo PDF que:
      ...
      2. Contenha o tipo_imposto no nome ('ISS' ou 'IR')
         — para IR usa regex \\bIR\\b para não confundir com partes de outras palavras
    ...
    for nome in sorted(arquivos):
        ...
        if tipo_imposto.upper() == "IR":
            # \bIR\b evita match em palavras como "IRRF", "DIRETORIA" etc.
            # mas também aceita "IR " separado por espaço/hífen
            if re.search(r"\bIR\b", nome_norm):
                return os.path.join(pasta, nome)
        else:
            # ISS: busca simples pela string
            if tipo_imposto.upper() in nome_norm:
                return os.path.join(pasta, nome)

    return None

# FIM Alterei aqui em 19/03/2026 — BUSCA DE ARQUIVO ISS/IR NA PASTA
...
class App:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Mesclador de Comprovantes de Pagamento  v8.0")
    ...
    def _processar(self):
        ...
        4. Para cada registro: busca comprovante, ISS, IR, certidões
        5. SE comprovante OK: prossegue com ISS/IR/certidões
           SE comprovante INEXISTENTE: bloqueia TUDO para aquele registro
        6. Mescla na ordem: Autorização + Comprovante + ISS + IR + Certidões
        7. Gera relatório final de erros

        ── Alterei aqui em 19/03/2026 ──────────────────────────────────
        ISS e IR agora são buscados como ARQUIVOS NA PASTA usando o
        prefixo numérico do nome da Autorização, e não mais como
        keywords dentro do PDF de comprovantes.
        ────────────────────────────────────────────────────────────────
        """
        ...
        for nome_aut in arqs_aut:
            ...
            # ── Alterei aqui em 19/03/2026 ──────────────────────────
            # O prefixo_numerico é a parte antes de "Autorizacao",
            # incluindo os traços e pontos do número da conta.
            # Ex: "05-144.005.261-9 " (com espaço à direita preservado)
            # Será usado para localizar os arquivos ISS e IR na pasta.
            prefixo_numerico = prefixo   # mesmo valor, nome explícito para clareza
            ...
            # ── Alterei aqui em 19/03/2026 ──────────────────────────────
            # Passos 9 e 10 agora buscam ARQUIVOS na pasta pelo prefixo
            # numérico, em vez de páginas por keyword no PDF de comprovantes.
            #
            # A busca acontece UMA VEZ por Autorização (não por registro),
            # pois todos os registros da mesma Autorização compartilham
            # o mesmo arquivo ISS/IR.
            #
            # Exemplo de arquivos buscados:
            #   ISS → "05-144.005.261-9 ISS - HRG RC- BOLETO OK.pdf"
            #   IR  → "05-144.005.261-9 IR - HRG RC- BOLETO OK.pdf"
            #
            # Se o arquivo não existir na pasta → status INEXISTENTE
            # (bloqueia mesclagem apenas se a coluna ISS/IR for obrigatória)

            arq_iss_pdf = buscar_arquivo_imposto(pasta_comp, prefixo_numerico, "ISS")
            arq_ir_pdf  = buscar_arquivo_imposto(pasta_comp, prefixo_numerico, "IR")

            if arq_iss_pdf:
                self.lok(f"Arquivo ISS localizado: {os.path.basename(arq_iss_pdf)}")
            else:
                self.ldbg(f"Arquivo ISS nao encontrado para prefixo '{prefixo_numerico.strip()}'")

            if arq_ir_pdf:
                self.lok(f"Arquivo IR localizado: {os.path.basename(arq_ir_pdf)}")
            else:
                self.ldbg(f"Arquivo IR nao encontrado para prefixo '{prefixo_numerico.strip()}'")
            # FIM Alterei aqui em 19/03/2026
            ...
            for r in regs:
                ...
                # ── Alterei aqui em 19/03/2026 ──────────────────────────
                # Passo 9 – verifica arquivo ISS (só se obrigatório)
                # Antes: buscava keyword "ISS" dentro do PDF de comprovantes
                # Agora: usa o arquivo ISS já localizado por nome na pasta
                pags_iss = []
                if precisa_iss:
                    if arq_iss_pdf:
                        pags_iss = [(arq_iss_pdf, -1)]   # inclui arquivo ISS completo
                        self.lok(f"Passo 9 - ISS: arquivo '{os.path.basename(arq_iss_pdf)}'")
                    else:
                        status["iss"] = "INEXISTENTE"
                        self.lwrn("Passo 9 - ISS INEXISTENTE (arquivo nao encontrado na pasta)")
                # FIM Passo 9
                # FIM Alterei aqui em 19/03/2026

                # ── Alterei aqui em 19/03/2026 ──────────────────────────
                # Passo 10 – verifica arquivo IR (só se obrigatório)
                # Antes: buscava keyword "IRRF" dentro do PDF de comprovantes
                # Agora: usa o arquivo IR já localizado por nome na pasta
                pags_irrf = []
                if precisa_irrf:
                    if arq_ir_pdf:
                        pags_irrf = [(arq_ir_pdf, -1)]   # inclui arquivo IR completo
                        self.lok(f"Passo 10 - IR: arquivo '{os.path.basename(arq_ir_pdf)}'")
                    else:
                        status["irrf"] = "INEXISTENTE"
                        self.lwrn("Passo 10 - IR INEXISTENTE (arquivo nao encontrado na pasta)")
                # FIM Passo 10
                # FIM Alterei aqui em 19/03/2026
                ...
                # Verifica se ainda há algum INEXISTENTE (ISS, IR ou certidões)
                ...
                # Autorização (completo) + Comprovante + ISS + IR + Certidões
                todas_pags = ([(caminho_aut, -1)]   # Autorização completa
                              + pags_comp            # página(s) do comprovante
                              + pags_iss             # arquivo ISS completo (se aplicável)
                              + pags_irrf            # arquivo IR completo (se aplicável)
